...
class Pix2Pix(BaseGan):
    ...
    def eval_on_dataset(self, save_dir, label_len):
        self.eval_mode()
        eval_result = []
        for idx, data in tqdm.tqdm(enumerate(self.dataloader), total=len(self.dataset)):
            true_idx = idx / label_len
            real_a, real_b, depth = data
            depth_label = np.nonzero(depth.to('cpu').numpy().flatten())[0][0].item()

            real_a = real_a.to(self.device).float().permute((0, 3, 1, 2))
            real_b = real_b.to(self.device).float().permute((0, 3, 1, 2))
            depth = depth.to(self.device).float()

            fake_b = self.gen(x=real_a, x1=depth)

            real_a = real_a.permute(0, 2, 3, 1)
            real_b = real_b.permute(0, 2, 3, 1)
            fake_b = fake_b.permute(0, 2, 3, 1)

            real_a = real_a.detach().to('cpu').numpy()[0]
            real_b = real_b.detach().to('cpu').numpy()[0]
            fake_b = fake_b.detach().to('cpu').numpy()[0]
            ...
            p1 = os.path.join(save_dir, '{}_REALA_{}.jpg'.format(true_idx, depth_label))
            p2 = os.path.join(save_dir, '{}_REALB_{}.jpg'.format(true_idx, depth_label))
            p3 = os.path.join(save_dir, '{}_FAKEB_{}.jpg'.format(true_idx, depth_label))
            io.imsave(p1, real_a)
            io.imsave(p2, real_b)
            io.imsave(p3, fake_b)

            eval_result.append((p1, p2, p3))
        return eval_result
...
if __name__ == '__main__':
    ...
    dataset_cfg = DatasetConfig('FlowerFull', size=IMG_SIZE, num=label_len, train=False)
    ...
    with open('./eval_result' + EXP_NAME + '_index.json', 'w') as f:
        json.dump(
            {'index': dirs}, f
        )
